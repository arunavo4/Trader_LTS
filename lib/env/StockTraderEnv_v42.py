"""
    Looks like I have made some fundamental mistake here while building the Environment here.

    So moving to a new action model Long neutral short
    Long(Buy) --> Long(Hold) --> Short (Sell)
    Short(Sell) --> Short(Hold) --> Neutral(Hold) --> Long(BUY)
        ...
        if action_type == 1 and self.market_open:  # buy
            if len(self.positions) == 0:
                ...
                if self.enable_logging and not self._is_auto_hold:
                    message = "{}: Action: {} ; Reward: {}".format(self._current_timestamp(), "Long --> Hold",
                                                                   round(reward, 3))
                    self.action_record = message
                    self.logger.info(message)

                if self.enable_logging:
                    message = "{}: Going Long --> Hold!".format(self._current_timestamp())
                    self.action_record = message
                    self.logger.info(message)
                ...
                reward += self.get_reward(profit_percent)
                reward = (reward + np.sign(reward)) ** 3  # So moving to exponential reward
                ...
                if self.enable_logging:
                    ...
                    self.position_record = "{}: Qty : {} ; Avg: {} ; Ltp: {} ; P&L: {} ; %Chg: {}".format(
                        ...
                        self.qty * -1,
                        round((mean(self.positions) if len(self.positions) > 0 else 0), 2),
            ...
            if len(self.positions) == 0:
                ...
                if self.enable_logging and not self._is_auto_hold:
                    message = "{}: Action: {} ; Reward: {}".format(self._current_timestamp(), "Short --> Hold",
                                                                   round(reward, 3))
                    self.action_record = message
                    self.logger.info(message)

                if self.enable_logging:
                    message = "{}: Short --> Hold!".format(self._current_timestamp())
                    self.action_record = message
                    self.logger.info(message)
