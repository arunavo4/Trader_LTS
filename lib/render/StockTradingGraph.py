...
class StockTradingGraph:
    ...
    def render(self, current_step, net_worths, initial_step, trades, window_size=200):
        ...
        self.fig.suptitle('Net worth: Rs:' + str(net_worth) + ' | Profit: ' + str(profit_percent) + '%')

        window_start = max(current_step - window_size, initial_step)
        ...
        date_col = pd.to_datetime(self.df['Date']).dt.strftime('%Y-%m-%d %H:%M:%S')
